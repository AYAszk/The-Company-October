<?php
      //$_SESSION -- it is a variable, actually special variable. It can also store data
   ...
   //check the lists
   // print_r($all_users); //print_r() is a builtin function in PHP only use for debugging and for testing purposes ナビバーの上に表示。データが三つあれば３と表示。
 ?>
<!DOCTYPE html>
<html lang="en">
<head>
      ...
      <div class="col-6">
        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
               <?PHP
                 // get/or fetch all the lists of users and assign it to $user property/variable
                   while($user = $all_users->fetch_assoc()){ //opening  tag
              ?> 
           ...
           <!--  If the user has an image, this code below will be use to display the image  -->
                ...
                ?>
     <!-- Icon from Font-awesome website...use to display icon,if the image of the user is not available -->
                <i class="fa-solid fa-user text-secondary d-block text-center dashboard-icon"></i>
                  <?php //HTMLとPHPが混ざっているから、
                  }
                  ?>
              </td>
                <td><?=$user['id']?></td>
